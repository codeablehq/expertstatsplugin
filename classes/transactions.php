...
class wpcable_transcactions {
			...
			if ( empty( $single_page['transactions'] ) ) {
				...
				foreach ( $single_page['transactions'] as $tr ) {
					...
					if ( ! ( $check_client[0]->totalrows > 0 ) ) {
						...
						// the API is returning some blank rows, ensure we have a valid client_id
						if ( $this->validate_row( $new_client['client_id'] ) ) {
					...
					// the API is returning some blank rows, ensure we have a valid client_id
					if ( $this->validate_row( $new_amount['task_id'] ) ) {
	...
	/**
	 * Check if this row contains an empty field
	 *
	 * @author Justin Frydman
	 *
	 * @param $field int - The field to check
	 *
	 * @return bool
	 */
	private function validate_row( $field ) {
